...
module.exports = Components.detect(function(context, components, utils) {
  ...
  var forbidden = configuration.forbidden || [];
  ...
  function isPropTypesUsage(node) {
    ...
    return isClassUsage || isStatelessFunctionUsage || isForbidden(node.object.name);
  }
  ...
  /**
   * Checks if the prop is forbidden
   * @param {String} name Name of the prop to check.
   * @returns {Boolean} True if the prop is forbidden, false if not.
   */
  ...
  function markPropTypesAsUsed(node, parentNames) {
    ...
    switch (type) {
      case 'direct':
        ...
        break;
      case 'destructuring':
        var nameForbidden = isForbidden(node.object && node.object.name);
        for (var k = 0, l = properties.length; k < l; k++) {
          if (hasSpreadOperator(properties[k]) || properties[k].computed) {
            continue;
          }
          var propName = nameForbidden ? properties[k].name : getKeyValue(properties[k]);
          var checkName = nameForbidden ? propName : 'props';

          var currentNode = node;
          allNames = [];
          while (currentNode.property && currentNode.property.name !== checkName) {
            if (propName !== currentNode.property.name) {
